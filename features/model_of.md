...
* A libadalang-based **model decorator** tool is used to merge these files into a
...
(2) The **model decorator** tool.
(3) A workflow for integrating the model decorator tool with the rest of the
toolchain.
...
Step (3) is outside the scope of this proposal.
...
* When analysing legacy code, one may want to avoid unnecessary changes in the originla sources.
...
The workflow presented in the summary aims to minimize the impact on the complation process. The only envisaged changes are ignoring the syntax of the
sidecar file (to avoid including it in compilation chain by accident) and the support for annotations denoting the original location of the source code fragments. Generating the decorated file happens outside the compiler.
...
package models My_Math is
   function Sqrt (X : Float) return Float
     with
       Model_Of => My_Math.Sqrt,
       Pre  => X >= 0.0,
       Post => Sqrt'Result >= 0.0;
end My_Math;
```

A sidecar .ads begins with `package models <package-name> is` instead of
`package <package-name> is`.  The `models` keyword signals to the tool that
this file supplies additional specification information and is never compiled
as a stand-alone unit.

Inside a `models` package, each decorated subprogram carries `Model_Of =>
<package>.<subprogram>` as an aspect.  The value must be a statically
denoted subprogram in the target package.  All other aspects on the same
subprogram declaration (`Pre`, `Post`, `Contract_Cases`, `Global`, etc.) are
the contracts to be injected.

### (2) The model decorator tool

The libadalang-based model decorator tool reads both the main .ads and the
sidecar .ads and produces the decorated .ads:
...
Integrating the model decorator tool with the rest of the toolchain is outside
...
skipped — for instance because a `Model_Of` directive is a resolved reference
...
```
decoration_unit ::=
    "package" "models" package_name "is"
...
Every `decorated_subprogram_declaration` must contain exactly one `Model_Of`
aspect.

### (2) Model decorator tool: resolution and synthesis

**Resolution rules for `Model_Of`**

The value of `Model_Of` must denote a subprogram declared in the package named
by the enclosing `package models` clause.  Overloaded names are disambiguated
by matching the parameter profile of the decorated declaration against the
candidates in the target package.  If no candidate matches, or more than one
candidate matches, the model decorator tool reports an error.
...
2. Parse the sidecar .ads; for each `decorated_subprogram_declaration`, resolve
   `Model_Of` against the main .ads declarations.
...
5. The `Model_Of` aspect is a synthesis directive and is **not** emitted in the
   decorated .ads.
...
A separate sidecar .ads with `package models` syntax makes this separation
syntactically unambiguous: build systems can exclude sidecar .ads files without
any special configuration by recognizing the `package models` keyword.

**Why `package models` syntax rather than a library-level pragma?**
...
* Introduces a sidecar .ads artifact and a model decorator step not part of
  standard Ada.
* Meaning of `Model_Of` is entirely tool-defined; the aspect has no runtime
  semantics and is rejected by a standard compiler unless hidden behind a
...
* **Child packages**: can a sidecar .ads file decorate subprograms in a child
  package specification?  If so, should the `package models` clause mirror the
  full dotted name?
