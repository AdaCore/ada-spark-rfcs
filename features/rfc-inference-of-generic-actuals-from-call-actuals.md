...
explicit actual, and the *candidate profile* is the profile of the called
subprogram of the denoted instance. A formal that has a default is inferred
like any other formal; the default applies only when inference yields nothing.
...
that remains overloaded) infers nothing. An inferable formal for which
inference yields nothing takes its default; every inferable formal shall
either have its actual determined or have a default.
